...
from losses.myloss import svd_std

torch.backends.cudnn.deterministic = True
torch.backends.cudnn.benchmark = False

parser = argparse.ArgumentParser(description='MPI3d experiment')
...
log_path = os.path.join(log_root, f'{global_stamp}_{args.name}_{args.tgt}_{args.seed}')
...
def train_erm(optimizer,iter_train,iter_val):
    ...
    for iter_num in range(1, num_iter + 1):
        ...
        total_loss = regression_loss#+std_loss
...
def train_mixup(optimizer,iter_train,iter_val):
    ...
    for iter_num in range(1, num_iter + 1):
        ...
        loss_std = std1+std2
        loss_svd = torch.abs(svd1[0]-svd2[0]) ### change different norm here
        total_loss = regression_loss + 1e-2*loss_svd + 1e-12*loss_std
