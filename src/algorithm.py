...
from losses import SRC, RegMetricLoss
...
def train_downstream(args,x_train,y_train,x_valid,y_valid,best_r2_model,ind_same,ind_diff,best_mse_model,need_shuffle,iteration,
    ...
    optim2 = Adam([
        ...
        {'params':base_align.parameters(),'lr':args.optimiser_args['lr']}
        # {'params': gaussian_approximation.parameters(), 'lr': args.optimiser_args['lr'] * 10}
    ])
    ...
    for epoch in range(args.num_epochs):
        ...
        for idx in range(iteration):
            ...
            loss_std1, loss_svd1, dist_map1 = base_align.svd_std(feats, Y1)
            loss_std2, loss_svd2, dist_map2 = base_align.svd_std(feats_mixup, mixup_Y)
            loss_std = loss_std1 + loss_std2
            loss_svd = torch.abs(loss_svd1[0] - loss_svd2[0]).mean()

            loss = loss_mse + args.std_weight * loss_std +args.svd_weight * loss_svd
