...
class Learner(nn.Module):
    ...
    def forward_mixup(self, x1, x2, lam=None,return_feats=False):
        x1 = self.block_1(x1)
        x2 = self.block_1(x2)
        x = lam * x1 + (1 - lam) * x2
        x = self.block_2(x)
...
class Learner_TimeSeries(nn.Module):
    ...
    def forward_mixup(self, x1, x2, lam,return_feats=False):
...
class Learner_Dti_dg(nn.Module):
    ...
    def forward_mixup(self,x1, x2, lambd,return_feats=False):
        drug_num = self.input_shape[0][0] * self.input_shape[0][1]
